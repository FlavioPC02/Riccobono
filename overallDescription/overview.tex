...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{Example+Of+Latex+Doc/Images/classDiagram.png}
...
\begin{figure}[H]
    ...
    \begin{minipage}[t]{0.48\textwidth}
        \centering
        \includegraphics[height=6cm]{Example+Of+Latex+Doc/Images/stateDiagram.png}
        \caption*{Immagine 1} % opzionale
    \end{minipage}
    \hfill
    % Seconda immagine
    \begin{minipage}[t]{0.48\textwidth}
...
\paragraph{State Diagram 2}

\subsection{Product functions}
\paragraph{User registration and authentication}
BBP shall allow cyclists to register or authenticate, if already registered, by providing their personal information and required credentials, such as email and password. Registered users will have access to additional features, such as recording their route and entering information about cycling routes.

\paragraph{Browse bike paths and their conditions}
BBP shall allow any user, registered or not, to specify an origin and destination to view the bike path(s) between these two points on a map. For each bike path, the system will display basic information such as its approximate location, length, and the most recent status information collected from users.

\paragraph{Trip recording and history management}
BBP shall allow registered users to record their cycling activities. During a trip, the system collects the data needed to compute basic statistics. Once the recording is stopped, the system shall store the trip and present a summary including information such as total distance, duration and average speed. Users shall be able to view their past trips and related statistics.
